...
def setup_distributed():
    if "RANK" in os.environ and "WORLD_SIZE" in os.environ:
        ...
        dist.init_process_group(
            ...
            rank=rank,
            # 避免 rank0 单独跑较长 Val/Test 时，其他 rank 等待超过默认 600s 后被 NCCL watchdog 杀掉。
...
def main():
    ...
    # ==================== epoch test control ====================
    # 在 YAML 的 training 下控制：
    #   test_every: 0  -> 训练过程中不跑 test，只在训练结束后 final test
    #   test_every: 1  -> 每个 epoch 都跑 test
    #   test_every: N  -> 每 N 个 epoch 跑一次 test
    #
    # 额外控制：
    #   test_ema_each_epoch: true/false
    #   test_verbose_each_epoch: true/false
    test_every = int(config["training"].get("test_every", 0))
    test_ema_each_epoch = bool(config["training"].get("test_ema_each_epoch", True))
    test_verbose_each_epoch = bool(config["training"].get("test_verbose_each_epoch", True))

    if rank == 0:
        if test_every <= 0:
            print("[Epoch-Test] disabled: only final test will be run after training.")
        else:
            print(f"[Epoch-Test] enabled: run test every {test_every} epoch(s).")
            print(f"[Epoch-Test] test_ema_each_epoch={test_ema_each_epoch}")
            print(f"[Epoch-Test] test_verbose_each_epoch={test_verbose_each_epoch}")
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        if do_epoch_test:
            ...
            test_student_result = evaluate_loader(
                ...
                verbose=test_verbose_each_epoch,
                report_mani_macro=True,
            )

            if ema_enabled and test_ema_each_epoch:
